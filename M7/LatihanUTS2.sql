...
BEGIN
 ...
 --     ORDER BY O.ORDER_ID;
        CURSOR BATAS1 IS
            ...
            FROM
                INVENTORIES I
                JOIN PRODUCTS P
                ON P.PRODUCT_ID=I.PRODUCT_ID JOIN ORDER_ITEMS OI
                ON OI.PRODUCT_ID=I.PRODUCT_ID
                JOIN ORDERS O
                ON O.ORDER_ID=OI.ORDER_ID
            WHERE
                O.ORDER_DATE BETWEEN TO_DATE('1-1-2016', 'dd-mm-yyyy')
                AND TO_DATE('1-3-2016', 'dd-mm-yyyy')
 ...
 -- DBMS_OUTPUT.PUT_LINE();
        FOR AA IN BATAS1 LOOP
            DBMS_OUTPUT.PUT_LINE(AA.IDPRODUK
                ||' | '
                ||AA.NAME
                ||' | '
                ||AA.JUMLAHAWAL
